...
from modules.tools import filter_tools_by_hint, load_prompt, summarize_tools
import asyncio
...
def select_decision_prompt_path(planning_mode: str, exploration_mode: Optional[str] = None) -> str:
    ...
    # Rohan's original prompt
    # conservative_prompt_file = "prompts/decision_prompt_conservative.txt"
    # Modified prompt (< 300 words)
    conservative_prompt_file = "prompts/decision_prompt_conservative_opt2.txt"
    exploratory_parallel_prompt_file = "prompts/decision_prompt_exploratory_parallel.txt"
    exploratory_sequential_prompt_file = "prompts/decision_prompt_exploratory_sequential.txt"

    if planning_mode == "conservative":
        print("Using conservative prompt file: ", conservative_prompt_file)
        return conservative_prompt_file
    elif planning_mode == "exploratory":
        if exploration_mode == "parallel":
            print("Using exploratory parallel prompt file: ", exploratory_parallel_prompt_file)
            return "prompts/decision_prompt_exploratory_parallel.txt"
        elif exploration_mode == "sequential":
            print("Using exploratory sequential prompt file: ", exploratory_sequential_prompt_file)
            return "prompts/decision_prompt_exploratory_sequential.txt"
    print("Using conservative prompt file: ", conservative_prompt_file)
    return conservative_prompt_file  # safe fallback
...
async def generate_plan(
    ...
    # To avoid the 429 Resource Exhausted error
    await asyncio.sleep(3)
    print("Waiting for 3 seconds before generating plan...")  
